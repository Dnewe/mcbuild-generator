...
from mcbuild_generator.utils.fs_io import write_json, write_csv
...
def count_used_blocks(
    builds_fp: List[str], use_cache=True, multiproc=True
):  # TODO useless use_cache (always true in this part)
    """
...
def ignore_rare_variants(count_df: pd.DataFrame, threshold=0.1, prop_level="block"):
    """
    Map rare variants to the most common variant
    """
    count_df["variant_prop_build"] = count_df["build_count"] / count_df.groupby(
        "base_block"
    ...
    most_common_map = count_df.loc[
        count_df.groupby("base_block")[f"variant_prop_{prop_level}"].idxmax()
    ...
    count_df.loc[mask, "new_block"] = count_df.loc[mask, "base_block"].map(
        most_common_map
    )
